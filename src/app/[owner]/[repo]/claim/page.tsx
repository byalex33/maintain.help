...
export default async function ClaimPage({ params }: { params: Promise<ClaimPageParams> }) {
  ...
  const permission =
    username && accessToken ? await checkClaimPermission(accessToken, owner, repo, username) : { eligible: false, permission: null };
  ...
  return (
    <div className="mx-auto max-w-lg px-4 py-12">
      ...
      <p className="mt-2 text-neutral-600 dark:text-neutral-400">
        We checked your GitHub permissions on this repository — your input here always overrides our inferred
        status.
      </p>
      ...
      {!permission.eligible ? (
        <div className="mt-6 rounded-lg border border-amber-200 bg-amber-50 p-4 text-sm text-amber-900 dark:border-amber-900 dark:bg-amber-950/40 dark:text-amber-300">
          GitHub reports your permission on this repository as{" "}
          <strong>{permission.permission ?? "none"}</strong>. Claiming requires admin or maintain access — ask an
          existing maintainer to grant you access, or make sure you&rsquo;re signed in with the right GitHub account.
        </div>
      ) : (
        <div className="mt-6">
          <ClaimForm
