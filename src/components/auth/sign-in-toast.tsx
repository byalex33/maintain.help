    ...
    <div aria-live="polite" aria-atomic="true" className="pointer-events-none fixed inset-x-4 bottom-4 z-50 md:left-auto md:w-96">
      {visible && isSignedIn ? (
        ...
        <div data-slot="system-alert-banner" className="pointer-events-auto relative grid grid-cols-[2.375rem_minmax(0,1fr)] items-start gap-x-3 gap-y-1 overflow-hidden rounded-[1.25rem] border border-white/70 bg-white/95 p-4 pr-12 text-neutral-950 shadow-[0_8px_32px_-4px_rgba(0,0,0,0.10)] backdrop-blur-xl motion-safe:animate-in motion-safe:fade-in motion-safe:slide-in-from-bottom-2 dark:border-neutral-700 dark:bg-neutral-900/95 dark:text-neutral-50">
          ...
          <Button type="button" aria-label="Dismiss notification" onClick={() => setVisible(false)} variant="ghost" size="icon" className="absolute top-1 right-1 size-10 rounded-full text-muted-foreground">
